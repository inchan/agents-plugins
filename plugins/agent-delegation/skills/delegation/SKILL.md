...
description: >
  ...
  한국어 트리거: 위임, 부탁, 맡겨, 다른 AI에게 물어봐, codex로, 코덱스, gemini로, 제미나이, claude로, 클로드, kimi로, 키미, qwen으로, 큐웬
...
| 우선순위 | 키워드 | 에이전트 |
|---------|--------|----------|
| 1 | 검색, search, 최신, 뉴스, playwright, e2e, 크롤링, 제미나이, gemini | `gemini` |
| 2 | 아키텍처, 설계, 리뷰, 분석, 복잡한, 추론, 계획, 코덱스, codex | `codex` |
| 3 | 번역, translate, 중국어, 일본어, kimi, 키미, moonshot, 대용량, 장문, qwen, 큐웬 | `generalist` |
| 4 | 클로드, claude, (기본값) | `claude-code` |

## 사용 흐름

1. 사용자가 작업을 설명하면 스킬이 적합한 에이전트를 선택
2. 사용자가 CLI를 직접 지정할 수도 있음 ("codex로 리뷰해줘", "코덱스로 분석해줘", "제미나이로 검색해줘")
